...
namespace fps::rendering {
   ...
   void draw_triangle(float x1, float y1, float x2, float y2, float x3, float y3, Color col, bool bFill/* = true*/) {
      ...
      // dy2 = (corners[2].y - corners[0].y) / std::fabs(corners[2].x - corners[0].x);
      ...
      auto const draw_half = [col](coord2d const& from1, coord2d const& from2, coord2d const& to1, coord2d const& to2) {
         auto dy1 = (to1.y - from1.y) / std::fabs(to1.x - from1.x);
         auto dy2 = (to2.y - from2.y) / std::fabs(to2.x - from2.x);
         for (float y = std::max(from1.y, from2.y); y < std::min(to1.y, to2.y); y+=1.0f) {
